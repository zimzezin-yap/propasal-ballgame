...
const STALL_PROGRESS_PX = 7;
const STALL_TIME_LIMIT = 0.45;
const MIN_DOWNWARD_SPEED = 62;
...
function resolvePegCollision(ball, peg) {
  ...
  if (!peg.bumper && ball.vy < 18) ball.vy = 18;
}

function keepBallMovingDown(ball, width, finishY, dt) {
  if (ball.y > ball.lastProgressY + STALL_PROGRESS_PX) {
  ...
  const slowOrRising = ball.vy < MIN_DOWNWARD_SPEED;
  ball.stallTime += slowOrRising ? dt : dt * 0.35;
  if (ball.stallTime < STALL_TIME_LIMIT) return;

  const direction = ball.x < width / 2 ? 1 : -1;
  ball.rescueCount += 1;
  ball.vx += direction * (96 + Math.random() * 72);
  ball.vy = Math.max(ball.vy, 285 + Math.random() * 95);
  ball.x += direction * (2 + Math.random() * 3);
  ball.y = Math.min(finishY - ball.r, ball.y + 14 + Math.min(ball.rescueCount, 4) * 8);
  if (ball.rescueCount >= 3) {
    ball.x = Math.max(42 + ball.r, Math.min(width - 42 - ball.r, ball.x + direction * 34));
    ball.y = Math.min(finishY - ball.r, ball.y + 38);
  }
  ball.lastProgressY = ball.y;
  ball.stallTime = 0;
}
...
function resolveBallCollision(a, b) {
  ...
  a.vy = Math.max(a.vy, 34);
  b.vy = Math.max(b.vy, 34);
}
...
function step(timestamp) {
  ...
  balls.forEach((ball) => {
    ...
    if (ball.y > 82 && ball.y < finishY - 24 && ball.vy < MIN_DOWNWARD_SPEED) {
      ball.vy += (MIN_DOWNWARD_SPEED - ball.vy) * 0.42;
    }
    keepBallMovingDown(ball, width, finishY, dt);
